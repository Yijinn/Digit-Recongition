...
def add_consts_with_placeholder():
    ...
    addConstant = tf.add(c1,c2)
    af = tf.add(addConstant,c3)
...
def my_perceptron(x):
    ...
    w = tf.get_variable(dtype=tf.float32, initializer=tf.ones_initializer(), shape=[x], name="w")
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    Xsize = X.get_shape().as_list()[1]


    w1 = tf.Variable(tf.random_normal([Xsize, hiddensize]) * tf.sqrt(1.0/Xsize))
    w2 = tf.Variable(tf.random_normal([hiddensize, outputsize]) * tf.sqrt(1.0/hiddensize))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    length = conv2.get_shape().as_list()[1]
    width = conv2.get_shape().as_list()[2]

    x_image = tf.reshape(conv2, [-1, length*width*convlayer_sizes[1]])

    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(x_image, Y, outputsize)
